...
export class ProductListComponent implements OnInit {
  products: PaginationPage<ProductTemplate> = {number: 1, size: 0, content: [{id: 1, name: 'test', price: 0}]};
  ...
  goToPage(pageNumber: number) {
    ...
    this.catalogService.getProducts(this.currentPage - 1, 8)
      .subscribe(products => {
        ...
        err => this.products = {number: 1, size: 0, content: [{id: 1, name: 'test', price: 0}]});
  }
